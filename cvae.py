...
class CVAE(nn.Module):
    ...
    def __init__(self, data_mananger, device, hidden_dim=256, latent_dim=64, epochs=1, lr=1e-3):
        ...
        # Encoder-Decoder
        self.enc = Encoder(self.input_dim, self.hidden_dim, self.latent_dim, self.n_classes)
        self.dec = Decoder(self.input_dim, self.hidden_dim, self.latent_dim, self.n_classes)

        # Optim and Criterion
        self.optim = optim.Adam(self.parameters(), lr=self.lr)
        self.criterion = CVAE_Criterion()
    ...
    def fit(self):
        ...
        for i in range(self.epochs):
            ...
            if best_test_loss > val_loss:
                ...
                torch.save(self.state_dict(), 'cvae_state.pt')
    ...
    def epoch_train(self):
        ...
        for i, data in enumerate(self.data_manager.train_loader):
            ...
            # Forward-Backward
            reconstruction, mu, log_var = self.forward(x, y)
    ...
    # --------------------------------------------------------------------------------
    @torch.no_grad()
    def sample(self, idx=None, inverse_norm=True):
        z = torch.randn(1, self.latent_dim).to(self.device)

        if idx:
            y = torch.tensor([[idx]])
        else:
            y = torch.randint(0, self.n_classes, (1,1))

        y = self.one_hot(y).to(self.device)
    ...
    def one_hot(self, idx):
        onehot = torch.zeros(idx.size(0), self.n_classes)
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, latent_dim, n_classes):
        ...
        self.linear = nn.Sequential(nn.Linear(input_dim+n_classes, hidden_dim),
                                    nn.ReLU(True))
...
class Decoder(nn.Module):
    ...
    def __init__(self, output_dim, hidden_dim, latent_dim, n_classes):
        ...
        # P(x|z)
        self.dec_latent = nn.Sequential(nn.Linear(latent_dim + n_classes, hidden_dim),
                                        nn.ReLU(True),
...
# --------------------------------------------------------------------------------
class CVAE_Criterion(nn.Module):

    # --------------------------------------------------------------------------------
    def __init__(self):
        super(CVAE_Criterion, self).__init__()
